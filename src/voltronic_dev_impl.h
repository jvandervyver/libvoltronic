...
#define __VOLTRONIC__DEV__IMPL__H__
  ...
  typedef int (*voltronic_dev_read_f)(void* impl_ptr, char* buffer, const size_t buffer_size, const unsigned long timeout_milliseconds);
  typedef int (*voltronic_dev_write_f)(void* impl_ptr, const char* buffer, const size_t buffer_size);
  typedef int (*voltronic_dev_close_f)(void* impl_ptr);
